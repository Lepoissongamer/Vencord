...
const settings = definePluginSettings({
    ...
    autoPerformanceCharThreshold: {
        ...
        isValid: value => Number(value) >= 1 || t("validation.minimumOne")
    },
...
async function saveFileToComputer(filename: string, fileText: string) {
    ...
    try {
        ...
        console.error("[VenCode] Failed to download edited file", err);
...
`;
    ...
    const paintEditorHighlights = (text: string, query: string, shouldSearch: boolean) => {
        ...
        void getShikiPlugin()?.shiki?.tokenizeCode?.(text, lang).then(tokens => {
            ...
            console.error("[VenCode] Failed to render editor Shiki layer", err);
        });
    };
    ...
    const renderViewer = () => {
        ...
        if (canUseShiki) {
            try {
                ...
                console.error("[VenCode] Failed to render Shiki", err);
            }
        }
